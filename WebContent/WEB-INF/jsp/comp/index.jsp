...
<jsp:include page="/WEB-INF/jsp/common/head.jsp"></jsp:include>
	...
	<script type="text/javascript">
		...
		var LOAD_ACTION="company.loadBar";
		var UPDATE_STATUS_REMARK="company.updateStatusRemark";
		var objid="";
		...
		var load = function(pageNo) {
			...
			services.call("company.countCompany", ["${param.cpname }", $("#quickSearch").val(),$("#quickSearch_name").val()], afterCount);
		};

		var afterCount = function(obj) {
			if (!obj.success) {
				alert("some errors occur while processing request, afterCount");
				return;
			}

			services.call("company.listCompany", [ currentPage, "${param.cpname }", $("#quickSearch").val(),$("#quickSearch_name").val()], afterLoad);
			$("#pageCtrl").append(pageCtrl(obj.data.count, currentPage));
		};
		
		var afterLoad = function(obj) {
			if(!obj.success) {
				alert("some errors occur while processing request,afterLoad");
				return;
			}
			for(var i=0; i<obj.data.length; i++) {
					...
					$( "#dialog-confirm" ).dialog({
						...
						buttons: {
							"Active/InActive": function() {
								services.call("company.activeCompany", [ id ], afterActive);
								$( this ).dialog( "close" );
								...
								$( "#dialog-confirm" ).dialog({
									...
									buttons: {
										"Synchronize": function() {										
											//$btn1.unbind('click');
											services.call("traveler.setDirty", [ companyNo, -1], afDirty);
											$( this ).dialog( "close" );
				...
				insertRow($("#selectable"), obj.data[i].id,  [ $btn, i + 1, a("cname", obj.data[i].companyNo, 
					"displayComp.action?id="+obj.data[i].id),
				...
				$("#"+obj.data[i].id).dblclick(function(){
					var id=$(this).attr("id");
					var companyNo=$("#"+id+" td A").html();
					
					if($(this).hasClass("obj_lock")){
						services.call("traveler.unlock", [ companyNo ], afUnlock);
					}else{
						services.call("traveler.lock", [ companyNo ], afLock);
					}
				});
				
				var id = $btn1.parent().parent().attr("id");
				var companyNo = $("#"+id+" td A").html();
				$btn1.attr("id", "btn_" + obj.data[i].companyNo);
			}
		...
		function afDirty(obj){
			if(obj==null){
				alert("Company is not found in MPP.");
				return;
			}
			var companyNo=obj.companyNo;
			services.call("traveler.synchronize", [ companyNo, 0, 1, "PS,BS,AXO"], afSync);
		}
		...
		function afSync(obj) {
			...
			for(i=0;i<citsamexSystems.length;i++) {
				...
				$status_div=$("#sync_status_" + obj.companyNo + citsamexSystems[i]);
				if($status_div.length==0){
					$status_div=$("<div/>",{id:"sync_status_" + obj.companyNo + citsamexSystems[i]});
		...
		$(function() {
			...
			$("#btn_loadBar").click(function(){
				$("#parload-no").attr("maxlength",6);
					...
					if($("#checkbox").hasClass("unchecked")){
						...
						$btn.click(function(){
							...
							$( "#dialog-confirm" ).dialog({
								...
								buttons: {
									"Delete": function() {
										services.call("company.deleteCompany", [ id ], afterDelete);
										$( this ).dialog( "close" );
									},
									Cancel: function() {
										$( this ).dialog( "close" );
			...
			var afterActive = function(obj){
				if (!obj.success) {
					alert("some errors occur while processing request, afterActive");
					return;
				}
			...
			var afterDelete = function(obj){
				if (!obj.success) {
					alert("some errors occur while processing request, afterDelete");
					return;
				}
				$("#" + obj.data).remove();
			};
		...
		function afCompSync(list) {
			
			for(i=0; i<list.length; i++) {
				
				var companyNo = list[i].companyNo;
				
				var dirty = list[i].dirty;
				var $t_btn=$("#btn_"+companyNo);
				...
				if( list[i].lock==1 && list[i].lockuser==cur_user && dirty != -1 ) {
					...
					for(j = 0; j < citsamexSystems.length; j++) {
						...
						$status_div=$("#sync_status_" + list[i].companyNo + citsamexSystems[j]);
						if($status_div.length==0){
							$status_div=$("<div/>",{id:"sync_status_" + list[i].companyNo + citsamexSystems[j]});
	...
	<body>
		...
		<jsp:include page="/WEB-INF/jsp/form/statusRemarkcomp.jsp"></jsp:include>
		<div id="statusshower">Loading...</div>
		<table width="100%" cellpadding="0" cellspacing="0">
			<tr>
				<td class="loginform" valign="top">
					<div id="btn_new" class="info_text">
						To create new company, please click <a href="createComp.action">new</a>.
					</div>
					
					<div id="btn_new2" class="info_text">
						To load from PowerSuite, please click <a id="btn_loadBar" href="#">load</a>.
					</div>
						...
						<tr>
							<td valign="top">
								<table id="header" width="100%" cellpadding="0" cellspacing="0">
									<tr>
										<td>Company No: 
											<input type="text" id="quickSearch" value="" />
											Company Name: 
											<input type="text" id="quickSearch_name" value="" />
